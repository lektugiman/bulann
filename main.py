    ...
    def solve_task(self):
        try:
            ...
            for idx, resource_id in enumerate(resource_ids, start=1):
                ress = self.session.post("https://www.binance.info/bapi/growth/v1/friendly/growth-paas/mini-app-activity/third-party/task/complete", 
                                          json={"resourceIdList": [resource_id], "referralCode": None}).json()
                if ress["code"] == "000000":
        ...
        try:
            while True:
                ...
                if self.game_response['code'] == '000000':
                    log("Game started successfully!", level="SUCCESS")  
                    return True
...
def run_account(index, token, proxy=None):
    if is_url_encoded(token):
        ...
        tokens = token
    log(f"=== Account {index} ===", level="SUCCESS")  
